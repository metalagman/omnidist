...
func writeWheel(wheelPath string, cfg *config.Config, uvDist config.DistributionConfig, target config.Target, version string, binaryData []byte) error {
	...
	if err := addZipFile(zipWriter, launcherPath, []byte(pythonLauncher(binaryName)), 0644); err != nil {
		return err
	}
	...
	if err := addZipFile(zipWriter, path.Join(distInfoDir, "entry_points.txt"), []byte(consoleEntryPoints(cfg.Tool.Name, distName)), 0644); err != nil {
		return err
	}
